...
enum OrderStage
{
	...
	ORDERDECLINED	=	2,		//	заказ отвергнут
	ORDERASSIGN	=	3,		//	сервис передал выбранному водителю уведолмление, но не получил подтверждения
	ORDERASSIGNED	=	4,		//	получил подтверждение от водителя, заказ принят
	ORDERCONFIRMED	=	5,		//	Пользователь согласился с временем ожидания, выставленного водителем
	ORDERDRIVING	=	6,		//	Водитель приступил к исполнению заказа (выехал к Пользователю)
	ORDERWAITING	=	7,		//	прибыл на место назначения, ожидает Пользователя
	ORDERTRANSPORTING=	8,		//	везет Пользователя к месту назначения
	ORDERSTOP	=	9,		//	Временная остановка
	ORDERCOMPLETE	=	10,		//	Заказ выполнен
...
struct CustomerStatistic
{
	...
	3:	ID customerid,		//	Внутреннее имя для поиска
	4:	TAG tag,		//	тег (группа)
...
struct ServiceOrder
{
	...
	24:	ID preferreddriverid,	//	ID предпочитаемого водителя в дополнение к предпочитемому Заказчиком (вторая очередь). НЕ ВОДИТЕЛЬ, а ID!
	25:	NUMBER32 totaltimefiscal,	//	чистое время в пути в минутах "по счетчику"
	26:	NUMBER32 totaltimeactual,	//	чистое время в пути в минутах, выставленное водителем
	27:	CURR sumfiscal,		//	сумма "по счетчику"
	28:	CURR sumactual,		//	сумма к оплате (выставленная водителем в минутах с учетом скидок и др.)
	29:	CabClass cabclass,	//	Класс экипажа
	30:	PaymentState paymentstate,
	31:	ClaimState claimstate,	//	жалоба есть, нет, закрыта
	32:	bool hasstops,		//	есть промежуточные остановки или точки доставки (приема документов курьером)
	33:	STR notes		//	пометки
}
...
struct NotificationEvent
{
	...
	4:	PersonRole emitterrole,			//	Роль отправителя
	5:	ID emitter,				//	Identifier отправителя
	6:	ServiceObject serviceobject,		// 	Объект, требующий внимания
	7:	ServiceAction serviceaction,		// 	Произошедшее действие
	8:	DATE datestart,				// 	Штамп времени, когда произошло или произойдет в будущем
	9:	bool infuture,				// 	Планируемое действие
	10:	ID serviceobjectid,			// 	Идентификатор объекта, требующий внимания
	11:	bool sent,				// 	Удачно отправлено
	12:	DATE sentdate,				// 	Штамп времени, когда отправлено уведомление
	13:	STR notes,				// 	Текстовые пометки
	14:	bool gcmsend,				// 	отправить нотификация driver/passenger
	15:	NUMBER32 gcmsent,			// 	1- Была отправлена нотификация driver/passenger, >1- повторы
	16:	bool isgcmsentsuccess,			// 	Была ли отправлена успешно нотификация driver/passenger
	17:	DATE gcmsentdate,			// 	время последней отправки извещения
	18:	STR gcmresponsecode,			// 	код последнего ответа GCM
	19:	STR gcmresponse				// 	последний ответ GCM (строка JSON обекта)
}
